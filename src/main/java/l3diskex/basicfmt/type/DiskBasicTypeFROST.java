...
public class DiskBasicTypeFROST extends DiskBasicTypeFAT8<DirectoryFrost> {
     ...
     * 使用可能なディスクサイズを得る
     *
     * @param disk_size  ディスクサイズ
     * @param group_size グループ数
     */
    @Override
     ...
     */
    @Override
    public int allocateUnitGroups(int fileunit_num, DiskBasicDirItem<DirectoryFrost> item, int data_size, AllocateGroupFlags flags, DiskBasicGroups[] group_items) throws IOException {
        int groups = 0;
        ...
        while (rc >= 0 && limit >= 0 && sizeremain > 0) {
            ...
            basic.getNumsFromGroup(group_num, next_group_num, basic.getSectorSize(), sizeremain, group_items[0]);
        ...
        if (rc >= 0) {
            if (flags == AllocateGroupFlags.ALLOCATE_GROUPS_APPEND) {
                ...
                if (group_items[0].size() > 0) {
                    rc = chainGroups(item.getStartGroup(0), group_items[0].get(0).group);
                }
            }
        } else {
            // グループを削除
     ...
     * @param div_num    分割番号 (can be null)
     * @param div_nums   分割数 (can be null)
     */
    @Override
     ...
     */
    @Override
    public int writeFile(DiskBasicDirItem<DirectoryFrost> item, InputStream istream, byte[] buffer, int size, int remain, int sector_num, int group_num, int next_group, int sector_end, int seq_num) throws IOException {
        int len = 0;
        if (remain <= size) {
            // 残り少ない
    ...
    public void getIdentifiedData(DiskBasicIdentifiedData data) {
        // タイトル名 FATエリア
        int[] div_num = new int[1], div_nums = new int[1];
