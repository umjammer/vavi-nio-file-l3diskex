...
public class DiskBasicTypeFLEX extends DiskBasicType<DirectoryFlex> {
    ...
    private FlexSirT flex_sir;   /** SIRエリア */
    ...
    public int getEmptyGroupNumber() throws IOException {
        ...
        int[] div_num_holder = new int[]{div_num};
        sector = basic.getSectorFromSectorPos(group_num, div_num_holder);
        if (sector == null) {
            // no free space ?
            return INVALID_GROUP_NUMBER;
        }
    ...
    public double checkFat(boolean is_formatting) throws IOException {
        ...
        for (int lsec_pos = dir_sta_lsec; lsec_pos <= dir_end_lsec; lsec_pos++) {
            ...
            FlexPtr p = new FlexPtr();
            byte[] bb = sector.getSectorBuffer(secofs);
            Serdes.Util.deserialize(new ByteArrayInputStream(bb), p);
    ...
    public boolean calcGroupsOnRootDirectory(int start_sector, int end_sector, DiskBasicGroups group_items) throws IOException {
        ...
        int div_nums = 1;
        // 開始セクタ
        int[] trk_num_holder = new int[]{trk_num};
        int[] sid_num_holder = new int[]{sid_num};
        int[] div_num_holder = new int[]{div_num};
        int[] div_nums_holder = new int[]{div_nums};
        DiskImageSector sector = basic.getManagedSector(start_sector - 1, trk_num_holder, sid_num_holder, null, div_num_holder, div_nums_holder);
        trk_num = trk_num_holder[0];
        sid_num = sid_num_holder[0];
        div_num = div_num_holder[0];
        div_nums = div_nums_holder[0];
        ...
        while (limit >= 0) {
            ...
            int current_div_num = div_num;
            byte[] buffer = sector.getSectorBuffer(secBufOfs(current_div_num + 1));
            FlexPtr p = new FlexPtr();
            byte[] bb = sector.getSectorBuffer(secBufOfs(current_div_num + 1));
            Serdes.Util.deserialize(new ByteArrayInputStream(bb), p);
            if (p == null) {
                valid = false;
                break;
            ...
            // 次のセクタを得る
            trk_num_holder[0] = trk_num;
            sid_num_holder[0] = sid_num;
            div_num_holder[0] = div_num;
            div_nums_holder[0] = div_nums;
            sector = basic.getSectorFromSectorPos(getSectorPosFromNumS(p.nextTrack & 0xff, p.nextSector & 0xff), trk_num_holder, sid_num_holder, div_num_holder, div_nums_holder);
            trk_num = trk_num_holder[0];
            sid_num = sid_num_holder[0];
            div_num = div_num_holder[0];
            div_nums = div_nums_holder[0];
        ...
        // 最終セクタ番号を更新
        int sec_pos = getSectorPosFromNum(trk_num, sid_num, sec_num, 0, 1);
...
//        fatAvailability.SetCount(basic.diskBasicParam.GetFatEndGroup() + 1, FAT_AVAIL_USED.getValue());
        ...
        while ((track_num != 0 || lsector_num != 0) && limit >= 0) {
            ...
            int[] div_num_holder = new int[]{div_num};
            int[] div_nums_holder = new int[]{div_nums};
            sector = basic.getSectorFromSectorPos(sector_pos, div_num_holder, div_nums_holder);
            div_num = div_num_holder[0];
            div_nums = div_nums_holder[0];
            ...
            if (random_file > 0) {
                ...
                for (int sec = 0; sec < random_file && !finished; sec++) {
                    ...
                    for (int pos = 4; pos < logSecSiz(isector.getSectorSize()); pos += StFlexFsm.getSize()) {
                        ...
                        int[] sec_num_fsm_holder = new int[]{sec_num_fsm};
                        getNumFromSectorPosS(ritem.group, trk_num_fsm_holder, sec_num_fsm_holder);
                        trk_num_fsm = trk_num_fsm_holder[0];
                        sec_num_fsm = sec_num_fsm_holder[0];

                        // Accessing struct StFlexFsm in byte array
                        // assuming a method to write the structure fields to the byte array
                        StFlexFsm fsm = new StFlexFsm();
                        fsm.track = (byte) trk_num_fsm;
                        fsm.sector = (byte) sec_num_fsm;
                    ...
                    // Accessing struct FlexPtr in byte array at offset 0
                    FlexPtr p = new FlexPtr();
                    Serdes.Util.deserialize(buf, p);
    ...
    public int chainGroups(int group_num, int append_group_num) throws IOException {
        ...
        int div_num = 0;
        int[] div_num_holder = new int[]{div_num};
        DiskImageSector sector = basic.getSectorFromSectorPos(group_num, div_num_holder);
        div_num = div_num_holder[0];
        if (sector == null) {
            // why?
            return -1;
        ...
        int[] next_track_num_holder = new int[]{next_track_num};
        int[] next_sector_num_holder = new int[]{next_sector_num};
        getNumFromSectorPosS(append_group_num, next_track_num_holder, next_sector_num_holder);
        next_track_num = next_track_num_holder[0];
        next_sector_num = next_sector_num_holder[0];
        p.nextTrack = (byte) next_track_num;
        p.nextSector = (byte) next_sector_num;
        ByteArrayOutputStream bb = new ByteArrayOutputStream();
        Serdes.Util.serialize(p, bb);
        byte[] dest = sector.getSectorBuffer();
        System.arraycopy(bb.toByteArray(), 0, dest, secBufOfs(div_num + 1), bb.size());
        ...
        for (int div_num = 0; div_num < basic.diskBasicParam.getGroupsPerSector(); div_num++) {
            ...
            if (track.getTrackNumber() > 0 && track.getSideNumber() < basic.diskBasicParam.getSidesPerDiskOnBasic()) {
                ...
                if (p != null) {
                    ...
                    System.arraycopy(baos.toByteArray(), 0, sector.getSectorBuffer(), secBufOfs(div_num + 1), baos.size());
                }
            }
        }
    }
    ...
    public boolean additionalProcessOnFormatted(DiskBasicIdentifiedData data) throws IOException {
        ...
        for (int lsec_pos = dir_sta_lsec; lsec_pos <= dir_end_lsec; lsec_pos++) {
            ...
            if (prev_sector != null) {
                FlexPtr p = new FlexPtr();
                byte[] bb = prev_sector.getSectorBuffer(secBufOfs(prev_lsec_pos + 1));
                Serdes.Util.deserialize(new ByteArrayInputStream(bb), p);
                if (p != null) {
                    ...
                    Serdes.Util.deserialize(p, src);
                    System.arraycopy(src.toByteArray(), 0, prev_sector.getSectorBuffer(), secBufOfs(prev_lsec_pos + 1), src.size());
                }
            }
    ...
    public boolean additionalProcessOnDeletedFile(DiskBasicDirItem<DirectoryFlex> item) throws IOException {
        ...
        if ((flex_sir.free_start_track == 0 && flex_sir.free_start_sector == 0)
            ...
            // チェインする
            int[] div_num_holder = new int[]{div_num};
            sector = basic.getSectorFromSectorPos(getSectorPosFromNumS(flex_sir.free_last_track & 0xff, flex_sir.free_last_sector & 0xff), div_num_holder);
            div_num = div_num_holder[0];
            if (sector == null) return false;
            p = new FlexPtr();
            byte[] b = sector.getSectorBuffer(secBufOfs(div_num + 1));
    ...
    public void remakeChainOnFreeArea() throws IOException {
        ...
        while (free_track_num != 0 && free_sector_num != 0) {
            ...
            int div_num = 0;
            int[] div_num_holder = new int[]{div_num};
            sector = basic.getSectorFromSectorPos(getSectorPosFromNumS(free_track_num, free_sector_num), div_num_holder);
            div_num = div_num_holder[0];
    ...
    public static class StFlexFsm {
        ...
        public static StFlexFsm readFromBuffer(byte[] buffer, int offset) {
            StFlexFsm obj = new StFlexFsm();
            int currentOffset = offset;
            obj.track = buffer[currentOffset++];
            obj.sector = buffer[currentOffset++];
            obj.count = buffer[currentOffset++];
            return obj;
        }

        public static void writeToBuffer(StFlexFsm obj, byte[] buffer, int offset) {
            int currentOffset = offset;
            buffer[currentOffset++] = obj.track;
            buffer[currentOffset++] = obj.sector;
            buffer[currentOffset++] = obj.count;
        }
    }
}